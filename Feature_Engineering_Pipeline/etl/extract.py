...
from Feature_Engineering_Pipeline import utils, settings
...
def from_api(
    ...
    """
    Extract data from the DK energy consumption APT.
    
    Args:
        export_end_reference_datetime: The end reference datetime of the export window. If None, the current time is used.
        days_delay: The data has a delay of N days. Thus we have to shift our window with N days.
        days_export: The number of days export.
        url: The URL of the API.
        datetime_format: The datetime format of the field in API response.

    Returns:
        A tuple of a pandas dataframe containing the exported data and dicitonary of metadata.        
    """
...
def _compute_extraction_window(export_end_reference_datetime: datetime.datetime, days_delay: int, days_export: int) -> Tuple[datetime.datetime, datetime.datetime]:
    ...
    if export_end_reference_datetime is None:
        # As the dataset expired in july 2023, we set the export end reference time as last day of June 2023.
